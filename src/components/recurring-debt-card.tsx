...
/** A recurring temporary expense (e.g. money owed to someone, paid back
 *  monthly) shown on the Crédits page next to real credits. Display only —
 *  every number comes from the existing engine and confirmed payments. */
export function RecurringDebtCard({
  expense,
  payments,
  byId,
  currency,
        ...
        <CardContent className="flex flex-col gap-3 pt-4">
          <div className="flex items-center justify-between gap-2">
            <h3 className="flex min-w-0 items-center gap-2 text-base font-semibold text-slate-900 dark:text-white">
              <span className="text-lg leading-none">{expenseIcon({ icon: expense.icon, type: expense.type })}</span>
